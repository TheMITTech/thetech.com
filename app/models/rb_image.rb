...
class RbImage < ActiveRecord::Base
  ...
  scope :search_query, lambda { |q|
    ...
    where(
      terms.map { |t|
        "(LOWER(rb_images.caption) LIKE ? OR LOWER(rb_images.attribution) LIKE ?)"
      }.join(' AND '),
  ...
  # Otherwise, we can change the web_status to whatever we like.
