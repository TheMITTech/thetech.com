class PreRebirthArticle < AbstractModel
  has_and_belongs_to_many :users, foreign_key: 'article_id'
  has_many :authors, through: :authorships, foreign_key: 'article_id'
  has_many :authorships, foreign_key: 'article_id'
  has_many :article_versions, foreign_key: 'article_id'
  ...
  def meta(name)
    case name
    when :headline, :subhead, :bytitle, :intro, :modified_at, :published_at, :syndicated?, :rss_content, :attribution
      self.send(name)
    when :piece
